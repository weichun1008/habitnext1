# Slice L — 科學佐證（Scientific Brief）系統
...
Slice L 是 **「給 PubMed paper，AI 生 brief 草稿、admin review、user 看成品」**（admin-facing AI，user 看 review 後內容）。

兩條都用 AI，但風險不同：Slice L 因為有人類審稿閘門，AI hallucination 不會直達使用者。
...
- **Daily/weekly AI report 介接**：Slice L 提供事實來源層，但 report 本身是 Slice N+（未定）。
...
1. **Task ↔ OfficialHabit 關聯**：當前 `Task` 是否有 `officialHabitId` 欄位？沒有的話，TaskDetailModal 顯示 insight 的 fallback 怎麼做（name 反查 / 不顯示 / Slice L+1 補欄位）？
2. **Admin 介面位置**：插在現有 `/admin/dashboard/habits/[id]` 詳情頁，還是獨立路徑 `/admin/dashboard/insights`？傾向**前者**（context-binding 強）。
3. **AI 模型版本**：固定 `claude-sonnet-4.7` 還是 env var 可換？建議 env var。
4. **Tag taxonomy**：自由 freeform vs 預定義列表（從 GENESIS+IO 9 domain + 一些常用標籤）？v1 freeform，未來看蓋了幾條後再 curate。
5. **detail markdown 渲染**：用既有 lib 還是新引一個？專案內有沒有 markdown renderer？需確認。
...
| **後續** | Slice L+1：Task.officialHabitId 欄位（若 §9.2.1 確認沒有） / RecommendationPanel teaser / report 介接 | — |
