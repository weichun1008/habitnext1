...
> ⚠️ **2026-06-06 前端隱藏**：TaskCard 完成任務時原本會顯示「加地點」(Slice O `LocationChip`) 與「記錄這餐」(Slice Q `MemoryCapture`) 兩個 chip。因這兩個功能尚未實作完成(照片上傳停用 no-op、定位 opt-in 也未過濾),PM 決定**前端先隱藏**——已移除 TaskCard 內該渲染區塊(元件本身保留,程式碼留有註解)。功能成熟要恢復顯示時,把 TaskCard 標題段下方的 Slice O/Q 區塊還原即可(建議恢復時加上條件:飲食類才顯示「記錄這餐」、開定位才顯示「加地點」)。
...
1. **使用者認證 / 授權 — 預計整合 cofit 既有會員系統**（**最大對外資安缺口，上線前必處理**）
   - 現況：phone + bcrypt 密碼，**無 session / JWT middleware**；前端把 `userId` 直接放 query string。
   - 風險（IDOR）：多數使用者資料 API（`/api/tasks?userId=`、`/api/journey?userId=`、`/api/memory/[id]?userId=`、`/api/user/profile`、`/api/aspirations*`、`/api/stats`…）**信任 client 傳來的 userId** → 任何人知道他人 `userId`（+ `historyId`）即可讀/改其資料。
   - **方向（已定）**：不自建一套登入，改**整合 cofit 會員系統**當身分提供者。模式同本次的後台授權（§0.5-C）——每個請求帶 cofit 簽發、伺服器可驗證的憑證，後端解析出身分，**不再信任 query 的 `userId`**。

   **(a) 待 cofit auth 團隊提供規格【待 cofit 提供】**（決定工法的前置依賴）：
   - cofit 對內部/第三方 App 提供的登入機制？（OAuth2 / OIDC / 簽章 JWT / 同網域共享 cookie / token 驗證 API）
   - 憑證如何驗證？（共享密鑰 HMAC / 公鑰 JWKS / 呼叫 cofit introspection endpoint）
   - 會員唯一識別（cofit member id）與 token 內含欄位；session 時效、登出、續期由誰管。

   **(b) habitnext1 側工作（規格確定後）**：
   - 新增 `requireUser(request)`（或 middleware，比照 `src/middleware.js`/`lib/adminAuth.js`）：從憑證解析 cofit member id → 對應本地 `User`。
   - **User 對應**：`User` 加 `cofitMemberId`（additive、唯一索引）；首次登入自動建立/連結帳號（provisioning）。
   - **全面替換信任 query 的端點**：上列所有 user 資料 API 改從 `requireUser` 取 userId、移除 `?userId=`；前端對應移除手動帶 userId。
   - 既有 phone+bcrypt 登入：決定保留（過渡）或關閉。

   **(c) 遷移**：現有 habitnext1 phone 帳號如何對應/綁定到 cofit 會員。

   > 在 (a) 規格到位前，此項為「**待整合**」而非「待自建」；habitnext1 側 (b)(c) 才能動工。
