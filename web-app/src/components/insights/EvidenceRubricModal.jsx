    ...
    <div className="fixed inset-0 z-[70] bg-black/50 flex items-end md:items-center justify-center" onClick={onClose}>
      ...
      >
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white">
          <h3 className="font-bold text-gray-800">{t('evidence.rubric.title')}</h3>
          <button onClick={onClose} aria-label={t('evidence.rubric.close')} className="p-1 hover:bg-gray-100 rounded-full transition-colors">
            <X size={20} className="text-gray-500" />
          </button>
        </div>
        <div className="p-5 space-y-4">
          <p className="text-xs text-gray-500 leading-relaxed">
            {t('evidence.rubric.intro')}
          </p>
          {DIMENSIONS.map((dim) => (
            <div key={dim.key}>
              <h4 className="font-bold text-gray-800 text-sm mb-1">{t(dim.labelKey)}</h4>
              <ul className="space-y-0.5">
                {dim.levels.map((l) => (
                  <li key={l.value} className="text-xs text-gray-600 flex justify-between">
                    <span>{t(l.labelKey)}</span>
                    <span className="text-gray-400">{t('evidence.rubric.points', { n: l.points })}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="pt-3 border-t border-gray-100 text-xs text-gray-600">
            <p>{t('evidence.rubric.totalScale')}</p>
            <p className="mt-1 font-medium">
              {t('evidence.rubric.tiers', {
                strongFrom: THRESHOLDS.strong,
                moderateFrom: THRESHOLDS.moderate,
                moderateTo: THRESHOLDS.strong - 1,
                preliminaryTo: THRESHOLDS.moderate - 1,
              })}
            </p>
          </div>
        </div>
