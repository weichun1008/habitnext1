...
// 階段名稱存 DB 為 zh-TW canonical（planBuilder PHASE_META）；顯示時依 id 反查 i18n key。
const PHASE_NAME_KEYS = { p1: 'plan.phases.p1', p2: 'plan.phases.p2', p3: 'plan.phases.p3' };
...
const SaveAsPlanModal = ({ isOpen, userId, aspirationId, defaultName, previewPlan, onClose, onSaved }) => {
  ...
  const submit = async (visibility) => {
    ...
    try {
      ...
      else { alert(t('plan.createFailed')); }
    } catch (e) { console.error('save plan error', e); alert(t('plan.genericError')); }
    ...
    <div className="fixed inset-0 z-[60] bg-black/50 flex items-end md:items-center justify-center p-0 md:p-4">
      <div className="bg-white w-full max-w-xl h-[88dvh] md:max-h-[88dvh] rounded-t-2xl md:rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100 flex justify-between items-center flex-shrink-0">
          <h2 className="text-base font-bold text-gray-800 flex items-center gap-1.5"><Layers size={16} className="text-emerald-500" /> {t('plan.saveAsPlanTitle')}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <label className="block text-xs font-bold text-gray-500 mb-1">{t('plan.nameLabel')}</label>
          ...
          <label className="block text-xs font-bold text-gray-500 mb-1">{t('plan.descriptionLabel')}</label>
          ...
          <p className="text-xs font-bold text-gray-500 mb-2">{t('plan.phasePreview', { n: phases.length })}</p>
          {phases.map((ph, i) => (
            <div key={ph.id || i} className="rounded-xl border border-gray-200 p-3 mb-2">
              <div className="flex items-center justify-between mb-1.5">
                <b className="text-[13px] text-gray-800">{PHASE_NAME_KEYS[ph.id] ? t(PHASE_NAME_KEYS[ph.id]) : ph.name}</b>
                <span className="text-[11px] text-gray-400 font-bold">{t('plan.phaseSummary', { days: ph.days, n: ph.tasks?.length || 0 })}</span>
              </div>
        ...
        <div className="px-5 py-3 border-t border-gray-100 flex gap-2.5 flex-shrink-0">
          ...
          <button type="button" onClick={() => submit('public')} disabled={submitting || !name.trim()}
            ...
            {submitting ? <Loader size={16} className="animate-spin" /> : null} {t('plan.sharePublic')}
          </button>
        </div>
      </div>
    </div>
