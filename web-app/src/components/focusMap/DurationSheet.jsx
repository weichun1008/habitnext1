  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/40 flex items-end justify-center" onClick={onClose}>
      <div className="bg-white w-full max-w-xl rounded-t-3xl p-5 pb-7 max-h-[80dvh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <h3 className="text-base font-extrabold text-gray-800 mb-3">{t('focusMap.duration.title')}</h3>
          ...
          {DURATION_OPTIONS.map(o => {
            ...
            return (
              <button key={keyOf(o.value)} type="button" onClick={() => onPick(o.value)}
                ...
                {o.recommended && <span className="absolute -top-2 right-2 bg-amber-500 text-white text-[8px] font-extrabold rounded-full px-1.5 py-0.5">{t('focusMap.duration.recommended')}</span>}
                {t(o.labelKey)}
                <span className="block font-medium text-[10px] text-gray-400 mt-0.5">{t(o.subKey)}</span>
              </button>
            );
          })}
        </div>
        ...
        <button type="button" onClick={() => setShowSci(s => !s)}
          aria-label={t('focusMap.duration.sciAria')}
          className="inline-flex items-center gap-1 mt-3 text-xs font-bold text-emerald-600">
          <FlaskConical size={13} /> {t('focusMap.duration.whyScience')} {showSci ? '▾' : '›'}
        </button>
        {showSci && (
          <div className="mt-2.5 rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-[11.5px] leading-relaxed text-emerald-800">
            {t(HABIT_FORMATION_SCIENCE.summaryKey)}
            <span className="inline-flex items-center gap-0.5 mt-1 font-bold text-emerald-600 cursor-pointer">{t('focusMap.duration.fullSource')} <ChevronRight size={12} /></span>
          </div>
        )}
